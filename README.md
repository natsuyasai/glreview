...
glreview は、lazygit 風のインターフェースで GitLab のマージリクエストをターミナルから操作できるツールです。GitLab リモートが設定されている git リポジトリのディレクトリで起動すると、カテゴリ別に MR を一覧表示し、差分のシンタックスハイライト表示やインラインコメントの投稿をターミナル上で行えます。
