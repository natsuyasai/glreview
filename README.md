...
glreview は、lazygit 風のインターフェースで GitLab のマージリクエストをターミナルから操作できるツールです。GitLab リモートが設定されている git リポジトリのディレクトリで起動すると、カテゴリ別に MR を一覧表示し、差分のシンタックスハイライト表示やインラインコメントの投稿をターミナル上で行えます。コメントはブラウザの GitLab UI と同様に、まず下書き（Draft Notes API）としてレビューに追加され、レビュー送信操作でまとめて公開されます。
...
- 差分の特定行へのインラインコメント、MR 全体へのノート、既存コメントへのリプライを下書きとしてレビューに追加し、まとめて送信(公開)
...
```
~/.config/glreview/config.toml
```

環境変数 `XDG_CONFIG_HOME` が設定されている場合は `$XDG_CONFIG_HOME/glreview/config.toml` に保存されます。
...
| `w` | 折り返し表示の切替 |
| `c` | 選択行にコメントをレビューに追加(下書き) |
| `Shift+R` | レビューを送信(下書きコメントを一括公開) |

### コメント入力

| キー | 操作 |
|---|---|
| `Ctrl+S` | レビューに追加(下書きとして保存) |
...
```
~/.config/glreview/logs/glreview_YYYY-MM-DD.log
```
